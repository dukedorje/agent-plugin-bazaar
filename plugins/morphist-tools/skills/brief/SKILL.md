---
name: brief
description: Write a short, high-signal brief for one unit of work — goal, testable acceptance criteria, external API contracts you had to look up, inherited gotchas, and explicit out-of-scope. Replaces heavyweight story templates. Use when starting a discrete piece of work, or when asked to "write a brief" or "spec this out".
user-invocable: true
...
The remaining failure modes are different, and only three things address them:
...
Everything else in a classic story — the As-a/I-want/So-that, the AC↔task cross-reference
matrix, the six-value status enum, the restatement of the architecture — is derivable
from the codebase or tracked in beads. Do not write it.
...
4. **Write the brief onto the bead** (`bd update`). `--out` only if
   named. Never `.omc/`.
...
- Anything learned the hard way while executing → append to `docs/LEARNINGS.md`.
  See `CONVENTIONS.md`. This is the single highest-value habit in the whole system:
  project-specific knowledge is exactly what a strong model cannot derive.
- Status lives in beads, not in the brief. Do not add a `Status:` line.
- Briefs are disposable. They are scaffolding for one unit of work, not a record.
  The durable artifacts are the code, the beads, `ARCHITECTURE.md`, and `LEARNINGS.md`.
