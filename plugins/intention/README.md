...
`advise` → `act` → `fold`, plus `brief` / `debrief` / `map` / `ready` / `run`). Canonical skill files live
...
skills add ./plugins/intention \
  --skill intend --skill change --skill advise --skill act --skill fold --skill brief --skill debrief --skill map --skill ready --skill run \
