...
- 2026-08-31 — Sol architecture-review is on when `OPENAI_API` or `OPENAI_API_KEY` is set. Packet-only via `spawn --adapter openai` (`gpt-5.6-sol`). Not a Codex slash. Default reader stays Opus 4.8. (`plugins/intention/scripts/ladder.py`, `spawn.py`)
