...
- 2026-09-02 — Folded `add-steer-verb` after map/debrief were already archived. Living lists kept `consult`/`status`; the delta's older skill roster was not copied over them. (`openspec/specs/verbs`)
...
- 2026-09-02 — OMC fileset coordinator is **ultrapilot** Phase 2 (ownedFiles / sharedFiles deferred / boundaryImports), not ultrawork (that's the parallel engine). Worktree isolate/land PARKED (`add-act-worktree-land`). Wave children stay on HEAD; persist sequentially after join. Sol consult dissented on stripping shared files in slice one — mutex/close still see the whole packet. (`openspec/parked.md`, `bazaar-7kb`, add-run-wave-workflow)
