...
`map`, `ready`, and `run` as the default planning loop. `run` is the
campaign; `map` and `ready` are observe; `brief` / `debrief` are
disposable decide; `steer` is human-gated guidance (not a campaign
wave).
...
only `change` → `advise` → `act`. `ready`, `map`, `brief`,
`debrief`, and `steer` SHALL stay outside that wave set.
