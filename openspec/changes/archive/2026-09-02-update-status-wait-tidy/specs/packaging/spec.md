...
`intend`, `change`, `advise`, `act`, `fold`, `brief`, `status`, and
`run`. `ready` SHALL remain as an alias skill that points at
`status`. Each `SKILL.md` SHALL have YAML frontmatter whose `name`
...
- THEN it discovers `intend`, `change`, `advise`, `act`, `fold`,
  `brief`, `status`, and `run` from `.agents/skills/`
